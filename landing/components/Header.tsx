    ...
    <header className="sticky top-0 z-[100] w-full glass-ios px-4 md:px-8 py-3 apple-shadow border-b border-white/20">
      <div className="max-w-7xl mx-auto flex justify-between items-center h-12">
        ...
        <nav className="hidden lg:flex items-center gap-8 text-sm font-semibold text-slate-600">
          <button 
            onClick={handleLogoClick} 
            className={`hover:text-blue-600 transition-colors ${currentView === AppView.LANDING ? 'text-blue-600' : ''}`}
          >
            Strona główna
          </button>
          <a 
        ...
        <div className="flex items-center gap-3 z-[110]">
          <button 
            onClick={() => { onNavigate(AppView.DEMO); setIsMenuOpen(false); }}
          ...
          >
            {currentView === AppView.DEMO ? 'Panel Kontrolny' : 'Darmowe Demo'}
          </button>
      ...
      <div className={`fixed inset-0 bg-white z-[100] lg:hidden transition-all duration-500 ease-in-out ${isMenuOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
        <div className="flex flex-col items-center justify-center h-full gap-8 p-6">
          <button onClick={handleLogoClick} className="text-2xl font-bold text-slate-900">Strona główna</button>
          <a href="#features" onClick={(e) => handleNavClick(e, 'features')} className="text-2xl font-bold text-slate-900">Funkcje</a>
          <a href="#pricing" onClick={(e) => handleNavClick(e, 'pricing')} className="text-2xl font-bold text-slate-900">Cennik</a>
          <a href="#faq" onClick={(e) => handleNavClick(e, 'faq')} className="text-2xl font-bold text-slate-900">FAQ</a>
          <button 
            onClick={() => { onNavigate(AppView.DEMO); setIsMenuOpen(false); }}
            className="w-full max-w-xs mt-4 px-8 py-4 bg-blue-600 text-white rounded-2xl font-bold text-xl shadow-xl active:scale-95"
          >
            Darmowe Demo
          </button>
        </div>
