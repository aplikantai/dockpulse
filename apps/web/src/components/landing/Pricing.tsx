      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-24 space-y-6">
          <h2 className="text-5xl font-black text-[#1d1d1f] tracking-tighter">
            Przejrzysty cennik. <br/> Bez ukrytych kosztow.
          </h2>
          <p className="text-slate-500 max-w-2xl mx-auto text-xl font-medium">
            Wybierz plan idealnie dopasowany do wielkosci Twojego zespolu.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 items-stretch">
          {PRICING_PLANS.map((plan, idx) => (
            ...
            >
              ...
              <Link
                href="/start"
                className={`w-full py-6 rounded-3xl font-bold text-xl transition-all active:scale-95 text-center ${
                  plan.recommended
                    ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-2xl shadow-blue-500/20'
                    : 'bg-[#f5f5f7] text-[#1d1d1f] hover:bg-slate-200'
                }`}
              >
                {plan.recommended ? 'Zacznij Okres Probny' : 'Wybierz Plan'}
              </Link>
            </div>
          ))}
        </div>
